...
subtitle: "Theming and Design: Colorful Simplicity"
...
When choosing the elements to include in my theme I wanted my graphs to have a clean, bright aesthetic. To this end, the backgrounds are white and most of the gridlines and axis lines are removed. Axis tick marks and numerical labels are left for ease of reading quantitative graphs, but legends are removed or labeled directly on the graph as much as possible. 

The colors chosen for this theme are bold but not dark colors. They are intended to make the graphs easier to read and to stand in stark contrast with the black-and-white other lines and text. The main color, a bright purple, is also used as a highlight color. It is present in every graph and helps tie them together. When other colors aren't necessary the purple is contrasted with a light grey color. I wanted the color choices to be easy to distinguish, but not necessarily denote something positive or negative. 
...
```{r}
t <- read.csv("Documents/Winter2019/data_viz/data/theme.csv")
t <- transform(t,color.ord  = factor( 
t$color, levels=c('Purple', 'Green', 'Orange', 'Blue', 'Teal', 'Yellow', "Grey"), ordered =TRUE))
t <- t[order(-xtfrm(t$color.ord)), ]

ggplot(data=t) +
geom_bar(stat="identity", aes(color.ord, val_y, group=1, fill=color.ord)) +
  scale_fill_manual(values=ncm_colors_wgrey) + xlab("") + ylab("") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        plot.title = element_text(hjust = 0.5, face = "bold"), 
        plot.subtitle = element_text(hjust=0.5), panel.background = element_rect(fill = "white"),
        legend.position="none", text=element_text(family=ncm_font), axis.ticks = element_blank(),
        axis.text.y = element_blank()) +
  labs(title="Color Palette for this Theme") + geom_hline(yintercept=6, colour="white", linetype="dashed") +
  annotate("text", x = 1, y = 3, label = "#C73245", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 2, y = 3, label = "#8AC437", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 3, y = 3, label = "#F65D3A", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 4, y = 3, label = "#4369EB", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 5, y = 3, label = "#32D59B", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 6, y = 3, label = "#EDA03C", family=ncm_font, fontface=2, color="white") +
  annotate("text", x = 7, y = 3, label = "#F4F4F4", family=ncm_font, fontface=2, color="darkgrey")
```
...
p + geom_tile(aes(fill = Crimes.Called.In)) + scale_fill_gradient2(low=ncm_contrast, high=ncm_highlight, name="Crimes Reported") +
...
```{r, echo=FALSE}
acd <- read.csv("Documents/Winter2019/data_viz/data/g5.csv")
...
ggplot(acd, aes(x=Percentage, y=rank, color=type)) +
  ...
  "#C732D5","#C732D5", "#8AC437", "#8AC437","#8AC437", "#8AC437","#8AC437","#8AC437"))) +
  geom_point(alpha=1, size=4) +
  scale_color_manual(values=c("#C732D5", "#8AC437", "#C732D5", "#8AC437"),
                     name="",
...
Each borough has a very different population, ranging from ~450k in Staten Island to 2.5 million residents in Brooklyn. These sizes makes them comparable to other small cities in their own right. In fact, Brookyln alone has a similar population to Chicago, the third largest city by population in the United States. It is therefore important to look at crime numbers in the context of where they happened. When looked at on a per capita basis, Manhattan and the Bronx have the most arrests per one hundred thousand residents. 

```{r, echo=FALSE}
ybc <- read.csv("Documents/Winter2019/data_viz/data/x.csv")
```
...
ggplot(ybc, aes(x=YR, y=ARRESTS_PER_100K, color=Borough, size=PERC_FELONIES)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
   ...
   labs(title="The most crimes per 100k residents take place in Manhattan and the Bronx",
        caption="Data Source: NYC Open Data Arrests Data") +
  expand_limits(x = c(2013, 2017.6)) +
...
annotate("text", x = 2017.1, y = 3140, label = "Brooklyn",  hjust=0, family=ncm_font, fontface=2, color="#8AC437") 
```
```{r, echo=FALSE}
g7 <- read.csv("Documents/Winter2019/data_viz/data/g7.csv")
