...
ncm_map_gradient <- c(low="#EFEDF5", mid="#C576D5", high=ncm_highlight) 
ncm_font <- "Helvetica"
...
ggplot(data = bdf) + facet_wrap(~year) +
  ...
  labs(title="Public Opinion Against Stop and Frisk Peaked in June 2012 with\nMarch to Mayor Bloomberg's residence", 
       subtitle = "Prevalence of the practice had decreased by the time it was deemed unconstitutional in August 2013.",
       caption="Data Source: NYPD Stop, Question, and Frisk Database") 


```

New York City is a diverse city, and about 25% of the population is black. However, throughout all boroughs the proportion of black detainees far surpassed the corresponding percentage of the population. The following two maps focus on the height of stop and frisk in 2011 and show this discrepancy; one for Manhattan and Brooklyn and one for the other three boroughs. 
...
manbronx <- ggplot(data = pdfm) + 
  geom_sf(color="black", aes(fill = percentage)) + 
  scale_fill_gradient2(low="#EFEDF5", mid="#C576D5", high=ncm_highlight, midpoint=75, name="Percentage of People Stopped\nWho Were Black") + 
  theme(panel.grid.major = element_line(colour = "white"), panel.grid.minor = element_blank(), 
        ...
        axis.ticks =element_blank(), legend.position = "bottom") +
  labs(title="In the majority of precincts the majority (> 50%) of people\ndetained in 2011 were Black\n", 
       subtitle = "Percentage of population that is black: \n17% in Manhattan, and 36% in the Bronx.",
       caption="Data Source: NYPD Stop, Question, and Frisk Database") 
...
```{r}
ggplot(data = pdfqbs) + 
  geom_sf(color="black", aes(fill = percentage)) + scale_fill_gradient2(low="#EFEDF5", mid="#C576D5", high=ncm_highlight, midpoint=50, name="Percentage of People Stopped\nWho Were Black") +
  theme(panel.grid.major = element_line(colour = "white"), panel.grid.minor = element_blank(), 
        plot.title = element_text(hjust = 0.5, face = "bold"), 
        plot.subtitle = element_text(hjust=0.5), panel.background = element_rect(fill = "white"),
        text=element_text(family=ncm_font), strip.background = element_rect(fill=ncm_contrast), strip.text = element_text(color="black"), axis.text = element_blank(), 
        axis.ticks =element_blank(), legend.position = "bottom") +
  labs(subtitle = "Percentage of population that is black: \n36% in Brooklyn, 20% in Queens,  and 10% in Staten Island.",
       caption="Data Source: NYPD Stop, Question, and Frisk Database") 
```
